...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Vector{ // vector math and physics if you prefer this over vector components on circles
        ...
        draw(){
            let dummy = new Point(this.object.x+(this.xmom*5), this.object.y+(this.ymom*5))
            let link = new LineOP(this.object, dummy, "", .25)
            if(Math.round(link.angle()*10000) > 0){
                link.color = toColor(Math.round(link.angle()*10000))
            }else{
                link.color = toColorx(Math.round(link.angle()*10000))
            }
            link.draw()
        }
    }
    ...
    function setUp(canvas_pass, style = "#000000") {
        ...
        window.addEventListener('pointerdown', e => {
            ...
            for(let t = 0;t<canvas.width;t++){
                for(let  k = 0;k<canvas.width;k++){                    
                    let link = new LineOP(TIP_engine, grid.vecs[t][k].object)
                    grid.vecs[t][k].xmom = Math.cos((grid.vecs[t][k].orginangle))
                    grid.vecs[t][k].ymom = Math.sin((grid.vecs[t][k].orginangle))
                }
            }
        ...
        function continued_stimuli(e) {
            ...
            for(let t = 0;t<canvas.width;t++){
                for(let  k = 0;k<canvas.width;k++){
                    let link = new LineOP(TIP_engine, grid.vecs[t][k].object)
                    // grid.vecs[t][k].xmom = Math.cos((grid.vecs[t][k].orginangle)+(link.angle()+Math.PI))
                    // grid.vecs[t][k].ymom = Math.sin((grid.vecs[t][k].orginangle)+(link.angle()+Math.PI))
                    grid.vecs[t][k].xmom -= Math.cos((link.angle()+Math.PI))*.1
                    grid.vecs[t][k].ymom -= Math.sin(((link.angle()+Math.PI)))*.1
                }
            }
        }
    }
...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class Grid{
        constructor(){
            this.vecs = []
            this.center = new Point(180,180)
            for(let t = 0;t<canvas.width;t++){
                let row = []
                for(let k = 0;k<canvas.height;k++){
                    let link = new LineOP(new Point(t,k), this.center)
                    let vec = new Vector((new Point(t,k)), Math.cos(link.angle()+(Math.PI*.5)), Math.sin(link.angle()+(Math.PI*.5)))
                    vec.orginangle = link.angle()
                    vec.normalize(1)
                    row.push(vec)
                }
                this.vecs.push(row)
            ...
            this.pixels = []
            for(let t = 0;t<100;t++){
                let pix = new Circle(Math.random()*canvas.width, Math.random()*canvas.width, 2, "red")
                // let pix = new Circle(180, 180, 2, "red")
                this.pixels.push(pix)
            } 
            // console.log(this.vecs)
        }
        draw(){
            for(let t = 0;t<this.pixels.length;t++){
                // console.log( this.vecs[Math.floor((this.pixels[t].y*canvas.width)+this.pixels[t].x)].xmom)

                // this.pixels[t].x%=canvas.width-1
                // this.pixels[t].y%=canvas.width-1
                if(this.pixels[t].x >= canvas.width){
                    continue
                }else if(this.pixels[t].y >= canvas.width){
                    continue
                }else{
                    if(typeof this.vecs[Math.floor(this.pixels[t].x)] !== "undefined" ){
                        if(typeof this.vecs[Math.floor(this.pixels[t].x)][Math.floor(this.pixels[t].y)] !== "undefined" ){
                        this.pixels[t].xmom = this.vecs[Math.floor(this.pixels[t].x)][Math.floor(this.pixels[t].y)].xmom
                        this.pixels[t].ymom = this.vecs[Math.floor(this.pixels[t].x)][Math.floor(this.pixels[t].y)].ymom
                        }
                    }
                }
            ...
            for(let t = 0;t<canvas.width;t++){
                for(let k = 0;k<canvas.height;k++){
                    ...
                    //             if(k < canvas.width-1){
                    //                 for(let h = 0;h<9;h++){
                    //                     // this.vecs[t][k].xmom += this.vecs[t+1][k-1].xmom
                    //                 }
                    //             }
                    //         }
                    //     }
                    // }
                    this.vecs[t][k].xmom *= .99
                    this.vecs[t][k].ymom *= .99
                    this.vecs[t][k].xmom += Math.cos(this.vecs[t][k].orginangle)*.01
                    this.vecs[t][k].ymom += Math.sin(this.vecs[t][k].orginangle)*.01
                    if((t*k)%10 == 0){
                        this.vecs[t][k].draw()
                    }
            }
